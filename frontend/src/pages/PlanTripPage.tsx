...
export default function PlanTripPage() {
  ...
  const handleSelectConcept = async (concept: TripConcept) => {
    if (!formData) return;
    setSaving(true);
    setError(null);
    ...
    try {
      const res = await fetch(`${API_URL}/api/trips`, {
        ...
        credentials: 'include', // vital for sending session cookie
        body: JSON.stringify({
          city: formData.city,
          ...
          conceptVibe: concept.vibeDescription,
        }),
      });

      if (!res.ok) {
        const errData = await res.json();
        throw new Error(errData.error || 'Failed to save trip');
      }
  ...
  return (
    <div className="min-h-screen bg-tripzy-bg text-on-surface font-body-md flex flex-col relative overflow-x-hidden">
      ...
      <main className="flex-grow w-full max-w-7xl mx-auto px-6 md:px-12 py-12 relative z-10 animate-fade-up">
        {error && (
          <div className="mb-6 p-4 rounded-xl bg-red-100 border-2 border-red-400 text-red-700 font-bold text-sm max-w-xl mx-auto">
            {error}
          </div>
        )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <section className="lg:col-span-7 animate-fade-up" style={{ animationDelay: '0.1s' }}>
              <div className="bg-white p-6 md:p-8 rounded-2xl border-[3px] border-black staggered-shadow relative z-20">
                <PlanTripForm 
                  onTopHalfComplete={setStepTwoActive} 
                  onConceptsLoaded={handleConceptsLoaded}
                />
              </div>
            </section>
          </div>
